...
export class MissionForm extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            file: null,
            filesError: null,
            sameOriginAndDestinationError: null,
            maximumPassengersError: null,
            noPassengersOrPilotsError: null
        }
    ...
    airplaneOptionsRender() {
        ...
        if (airplanes) {
            return airplanes.map((airplane, index) => {
                const airplaneText = airplane.airplaneModel.manufacturer.name + ' ' + airplane.airplaneModel.name + ' - ' + airplane.subscriptionNumber;
    ...
    mappedPassengers() {
        ...
        if (passengers) {
            return passengers.map((passenger, index) => {
                return {text: passenger.name, value: index, passenger: passenger}
            })
        } else {
            return [];
        }
    ...
    mappedPilots() {
        ...
        if (pilots) {
            return pilots.map((pilot, index) => {
                return {text: pilot.name, value: index, pilot: pilot}
            })
        } else {
            return [];
        }
    ...
    renderCurrentPassengersForUpdate() {
        ...
        if (missionPassengers) {
            return missionPassengers.map((passenger) => {
                return <Chip style={chipStyle} key={passenger.passenger.id} onRequestDelete={this.handleDeleteSelectedPassenger.bind(this, passenger.passenger)}>
    ...
    renderSelectedPassengers() {
        ...
        return this.state.selectedPassengers.map((passenger) => {
            return <Chip style={chipStyle} key={passenger.id} onRequestDelete={this.handleDeleteSelectedPassenger.bind(this, passenger)}>
                {passenger.name}
            </Chip>
        })
    }
    ...
    renderCurrentPilotsForUpdate() {
        ...
        if (missionPilots) {
            return missionPilots.map((pilot) => {
                return <Chip style={chipStyle} key={pilot.pilot.id} onRequestDelete={this.handleDeleteSelectedPilot.bind(this, pilot.pilot)}>
                    {pilot.pilot.name}
                </Chip>
            })
        }
    ...
    renderSelectedPilots() {
        ...
        return this.state.selectedPilots.map((pilot) => {
            return <Chip style={chipStyle} key={pilot.id} onRequestDelete={this.handleDeleteSelectedPilot.bind(this, pilot)}>
                {pilot.name}
            </Chip>
        })
    }
    ...
    submitData(event) {
        ...
        const airplaneObject = JSON.parse(event.target.airplane.value);
        const newMission = {
            mission: {
        ...
        //check if the maximum number of passenger was set..
        if (newMission.passengers.length > newMission.mission.airplane.seatsNumber) {
            this.setState({maximumPassengersError: 'Número máximo de passageiros da aeronave ultrapassado!'})
            return;
        }
        if (newMission.mission.missionTo === newMission.mission.missionFrom) {
            this.setState({sameOriginAndDestinationError: 'Selecione um destino diferente da origem'})
            return;
        }
        if (newMission.passengers.length === 0 || newMission.pilots.length === 0) {
            this.setState({noPassengersOrPilotsError: 'Uma missão precisao de ao menos um piloto ou passageiro'})
            return;
        }
    ...
    handleUpdateFrom(autocompleteResult, e) {
        this.setState({selectedFrom: autocompleteResult.airport})
    }

    handleUpdateTo(autocompleteResult) {
        this.setState({selectedTo: autocompleteResult.airport})
    }

    handleUpdatePassenger(autocompleteResult) {
        let actualSelectedPassengers = this.state.selectedPassengers;
        this.setState({selectedPassenger: autocompleteResult.passenger})
        ...
        if (actualSelectedPassengers.indexOf(selectedPassengerFromState) < 0) {
            ...
            this.setState({selectedPassengers: actualSelectedPassengers, passengersSearchInput: ' '})
        }
    }

    handleUpdatePilot(autocompleteResult) {
        let actualSelectedPilots = this.state.selectedPilots;
        this.setState({selectedPilot: autocompleteResult.pilot})
        ...
        if (actualSelectedPilots.indexOf(selectedPilotFromState) < 0) {
            ...
            this.setState({selectedPilots: actualSelectedPilots, pilotsSearchInput: ' '})
        }
    }
    ...
    parseAirplaneForSelect(airplane) {
        return airplane.airplaneModel.manufacturer.name + ' - ' + airplane.airplaneModel.name + ' - ' + airplane.subscriptionNumber;
    }
    ...
    editPilots() {
        ...
        this.setState({selectedPilots: mappedMissionPilots})
    }
    ...
    editPassengers() {
        ...
        this.setState({selectedPassengers: mappedMissionPassengers})
    }
    ...
    render() {
        ...
        const {params} = this.props;
        const {maximumPassengersError, sameOriginAndDestinationError, noPassengersOrPilotsError} = this.state;
        const {
            mission,
            newMission,
            inserting,
        ...
        return (
            <Container>
                ...
                <Form onSubmit={this.submitData}>
                    <DatePicker hintText="Data" floatingLabelText="Data*" fullWidth={true} name="date" defaultDate={params.id
                        ? this.currentDateTime()
                        : new Date()}/>
                    <TimePicker hintText="Hora" floatingLabelText="Hora*" defaultTime={params.id
                        ? this.currentDateTime()
                        : new Date()} format="24hr" fullWidth={true} name="time"/>
                    <Input label="Objetivo*" floatingLabel={true} required={true} name="reason" defaultValue={params.id
                        ? mission.reason
                        : ''}/>
                    <Select name="airplane">
                        {params.id
                            ? <Option label={this.parseAirplaneForSelect(mission.airplane)} value={JSON.stringify(mission.airplane)}/>
                            : null}
                        {this.airplaneOptionsRender()}
                    </Select>
                    <AutoComplete hintText="Origem" floatingLabelText="Origem*" dataSource={this.mappedAirports()} onNewRequest={this.handleUpdateFrom} searchText={params.id
                        ? mission.missionFrom.acronym + ' - ' + mission.missionFrom.name
                        : ''} fullWidth={true} errorText={params.id
                        ? null
                        : this.missionFromText()}/>
                    <AutoComplete hintText="Destino" floatingLabelText="Destino*" dataSource={this.mappedAirports()} onNewRequest={this.handleUpdateTo} searchText={params.id
                        ? mission.missionTo.acronym + ' - ' + mission.missionTo.name
                        : ''} fullWidth={true} errorText={params.id
                        ? null
                        : this.missionToErrorText()}/>
                    <AutoComplete hintText="Passageiro" floatingLabelText="Passageiro*" searchText={this.state.passengersSearchInput} dataSource={this.mappedPassengers()} onNewRequest={this.handleUpdatePassenger} fullWidth={true} errorText={this.state.selectedPilots.length < 0
                        ? 'Escolha ao menos um passageiro'
                        : null}/>
                    <div style={chipsWrapper}>
                        {showCurrentPassengers
                            ? this.renderCurrentPassengersForUpdate()
                            : null}
                        {showCurrentPassengers
                            ? <a className="mui-btn mui-btn--raised" onClick={this.editPassengers.bind(this)}>Editar Passageiros</a>
                            : null}
                    </div>
                    <div style={chipsWrapper}>
                        {showCurrentPassengers
                            ? <span>Novos passageiros</span>
                            : null}
                        {this.renderSelectedPassengers()}
                    </div>
                    <AutoComplete hintText="Piloto" floatingLabelText="Piloto*" searchText={this.state.pilotsSearchInput} dataSource={this.mappedPilots()} onNewRequest={this.handleUpdatePilot} fullWidth={true}/>
                    <div style={chipsWrapper}>
                        {showCurrentPilots
                            ? this.renderCurrentPilotsForUpdate()
                            : null}
                        {showCurrentPilots
                            ? <a className="mui-btn mui-btn--raised" onClick={this.editPilots.bind(this)}>Editar Pilotos</a>
                            : null}
                    </div>
                    <div style={chipsWrapper}>
                        {showCurrentPilots
                            ? <span>Novos pilotos</span>
                            : null}
                        {this.renderSelectedPilots()}
                    </div>
                    <Files onChange={this.handleUpdateFiles} onError={this.handleUpdateFilesError} maxFileSize={10000000} minFileSize={0} maxFiles={1} clickable>
                ...
                {inserting
                    ? <Formfeedback message={"Inserindo nova missão.."} duration={3000}/>
                    : null}
                {newMission
                    ? <Formfeedback message={"Nova missão agendada com sucesso"} duration={3000}/>
                    : null}
                {uploaded
                    ? <Formfeedback message={"Arquivo enviado com sucesso!"} duration={3000}/>
                    : null}
                {error
                    ? <Formfeedback message={"O erro " + {
                            error
                        } + " aconteceu"} duration={3000}/>
                    : null}
                {maximumPassengersError
                    ? <Formfeedback message={maximumPassengersError} duration={3000}/>
                    : null}
                {sameOriginAndDestinationError
                    ? <Formfeedback message={sameOriginAndDestinationError} duration={3000}/>
                    : null}
                {noPassengersOrPilotsError
                    ? <Formfeedback message={noPassengersOrPilotsError} duration={3000}/>
                    : null}
            </Container>
        )
    }
