/**
  A component that renders a Form for the Airplane Entity
  @see Airplane (Java)
**/
...
@connect((Store) => {
    return {airplaneModels: Store.airplaneModelsReducer, airplanes: Store.airplaneReducer}
})
            ...
            <div>
                <Container>
                    ...
                    <Form onSubmit={this.submitData}>
                        <Input label="Matrícula*" required={true} floatingLabel={true} name="subscriptionNumber" defaultValue={params.id
                            ? airplane.subscriptionNumber
                            : ''}/>
                        <TextField hintText="Numero de Assentos*" type="number" fullWidth={true} name="seatsNumber" defaultValue={params.id
                            ? airplane.seatsNumber
                            : ''} onChange={this.setSeatsNumber}/>
                        <div className="mui-select">
                            <select name="airplaneModel">
                                {params.id
                                    ? <option label={this.parseAirplaneForSelect(airplane)} value={JSON.stringify(airplane.airplaneModel)}/>
                                    : null}
                    ...
                    {updating
                        ? <Formfeedback message={"Atualizando nova aeronave.."} duration={3000}/>
                        : null}
                    {newAirplane
                        ? <Formfeedback message={"Aeronave " + newAirplane.subscriptionNumber + " inserida com sucesso"} duration={3000}/>
                        : null}
                    {updatedAirplane
